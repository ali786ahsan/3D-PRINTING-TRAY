...
#from sps30 import SPS30 # <-- note! this import causes datetime to report incorrect timezone!? what's going on!?!
import time
from datetime import datetime
import os
import sys

print('datetime={0}'.format(datetime.now()))
...
myDatetimeStr=datetime.now().strftime("%Y_%m_%d__%H_%M_%S") # capture datetime before SPS30 import!
print('myDatetimeStr={0}'.format(myDatetimeStr))

# NOTE: do not move this import up because it causes datetime to report in a different timezone
from sps30 import get_usb
from sps30 import SPS30 # <-- note! this import causes datetime to report incorrect timezone!? what's going on!?!
...
fname = '{0}_Sensirion_sps30_{1}.csv'.format(myDatetimeStr, SerialNumberStr)
print("fname={0}".format(fname))
try:
    print('opening logfile: {0}'.format(fname))
    file = open(fname,'w')
except:
    print('barf! could not open logfile: {0}'.format(fname))
    exit(-1)
...
while True:
    ...
    nBytes=file.write(SerialNumberStr + dateStr + dataStr + "\n")
    file.flush()
    print('just wrote [{0}] bytes to logfile: {1}'.format(nBytes,fname))
    time.sleep(1)
